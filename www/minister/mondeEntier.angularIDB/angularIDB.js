(function () {
    ...
    angular.module('mondeEntier.angularIDB', [])
    ...
    function angularIDB() {
                ...
                function setupTransaction(_store, _readWrite) {
                    ...
                    _store            = _store||store;
		                ...
		                if ($idb.db.objectStoreNames.contains(_store) !== true) {
			                console.error("No store iFound: " + _store);
			                return false;
		                }
                    ...
                    function aborted(e){
                      if(options.console.debug) console.log("Transaction has been aborted");
                    }

                    function completed(e) {
                        if(options.console.debug) console.info("Successful transaction on "+_store);
                    }
        ...
        function init(dbName, dbVersion, options) {
	        that.dbName         = dbName||'localIDB';
	        that.dbVersion      = dbVersion||1;
	        that.responseType   = options.responseType||"json";
	        that.reset          = options.reset||false;
          if (that.reset) {indexedDB.deleteDatabase(that.dbName); sky.info("Dropping iDB")}

	        if(api.Worker) {
		        emitWorker({"call": "upgrade"}, runIndexDB);
	        } else {
		        runIndexDB()
	        }

	        function runIndexDB(data, aWorker) {
        ...
        function emitWorker(_options, callback){
            ...
            angular.extend(values, _options);
            aWorker.postMessage(values);
            ...
            aWorker.addEventListener('error',function(e){
                console.error("Worker return error:: "+e.message,e);
            },false);
        }
    }
})();
